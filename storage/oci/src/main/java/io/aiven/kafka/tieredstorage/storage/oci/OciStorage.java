...
public class OciStorage implements StorageBackend {
    ...
    public void configure(final Map<String, ?> configs) {
        if (objectStorageClient != null) {
            throw new IllegalStateException("OciStorage is already configured");
        }
    ...
    // Visible for testing
    OciStorage(final ObjectStorageClient objectStorageClient,
    ...
    // No-arg constructor for Kafka plugin instantiation
    public OciStorage() {
    }
    ...
    public void delete(final ObjectKey key) throws StorageBackendException {
        try {
            ...
            // OCI throws 404 when deleting a non-existent object.
            // Treat delete as idempotent, consistent with S3 behavior.
    ...
    public InputStream fetch(final ObjectKey key, final BytesRange range) throws StorageBackendException {
        try {
            ...
            final GetObjectRequest getObjectRequest = GetObjectRequest.builder()
                ...
                .bucketName(bucketName)
                .objectName(key.value())
                .range(new Range((long) range.firstPosition(), (long) range.lastPosition()))
                .build();
            return objectStorageClient.getObject(getObjectRequest).getInputStream();
        } catch (final BmcException e) {
            if (e.getStatusCode() == 404) {
